...
<script type="text/javascript">
	function sbSubClick() {
		document.getElementById('sbSub').click();
	}
	function qxsbSubClick() {
		document.getElementById('qusbSub').click();
	}
</script>
</head>
<body>
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="PersonSB_personlist?type=${type }" method="post" id="ftijao">
	<div class="searchBar">
		<%@include file="form.jsp" %>
	</div>
	</form>
</div>
		<form action="PersonSB_personSBDo" method="post" onsubmit="return validateCallback(this,navTabAjaxDone)" id="saas">
		<div class="panelBar">
			<ul class="toolBar">
					<li><a class="search" href="Person_showPerson?pid={pid}"
						target="navTab"><span>查看</span></a></li>
					<c:if test="${type=='1' && fn:contains(menusids,',185,')}">
						<li>
						<a class="icon" onclick="sbSubClick();"><span>上报</span></a>
						<div style="display:none;">
							<input  class="search" id="sbSub" type="submit" value="上报">
						</div></li>
					</c:if>
					<c:if test="${type=='2' && fn:contains(menusids,',186,')}">
						<li>
						<a class="icon" onclick="qxsbSubClick();"><span>取消上报</span></a>
						<div style="display:none;">
						<input  class="search" id="qusbSub"type="submit" value="取消上报">
						</div>
						</li>
					</c:if>
			</ul>
...
<form id="pagerForm" action="PersonSB_personlist?type=${type}" method="post">  
