...
<table class="searchContent">
			<tr>
				<td>
					...
					<!-- 组织级别:
						<select name="zzjb" style="200px">
							<c:forEach items="${property}" var="p">
								<c:if test="${p.p_name=='组织级别'}">
									<option value="${p.id}">${p.content}</option>
								</c:if>
							</c:forEach>
						</select>
					安排职务:
						<select name="apzw" style="200px">
							<c:forEach items="${property}" var="p">
								<c:if test="${p.p_name=='安排职务'}">
									<option value="${p.id}">${p.content}</option>
								</c:if>
							</c:forEach>
						</select>
					安排界别:
						<select name="apjb" style="200px">
							<c:forEach items="${property}" var="p">
								<c:if test="${p.p_name=='安排界别'}">
									<option value="${p.id}">${p.content}</option>
								</c:if>
							</c:forEach>
						</select> -->
